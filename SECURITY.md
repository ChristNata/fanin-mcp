...
- **Prompt injection via upstream-provided text.** Tool names, descriptions, and results from upstreams are read by the LLM by design. The aggregator sanitizes upstream-authored tool names and descriptions in `list_tools` rows and the annotation strings (`title`/`description`/`$comment`) in `get_tool_schema` — neutralizing C0/C1 control characters, DEL, Unicode line/paragraph separators, bidi-override and zero-width characters — and length-caps each `list_tools` description row. This *bounds* the channel; it cannot eliminate it. By design, schema *validation* strings (`enum`, `const`, `default`, `pattern`, …) and tool-call *result* content pass through unmodified to stay byte-faithful, so they remain a residual (bounded, documented) channel. Treat upstream servers with the same trust you'd give a dependency.
