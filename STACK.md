...
|-------|---------|--------------|
| `rmcp` (exact pin) | MCP protocol — **both** roles | Official SDK; features: `server`, `client`, `transport-io`, `transport-child-process`, and `transport-streamable-http-client` + `transport-streamable-http-client-reqwest` for loopback `http://` Streamable-HTTP upstreams (the `-client` variant — bare `transport-streamable-http` is the server side). No TLS backend is linked in v1.0, so HTTPS/remote Streamable-HTTP upstreams are not reachable natively; use stdio/npx. The proxy is a server downstream and a client upstream in one process |
